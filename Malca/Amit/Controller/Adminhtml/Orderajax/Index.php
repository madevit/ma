...
namespace Malca\Amit\Controller\Adminhtml\Orderajax;
...
class Index extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($uname == '') {
            ...
            if (isset($params['isFilter'])) {
                ...
                if ($filtertype == 'order') {
                    $ordersarr->addFieldToFilter('status', $filter);
                    $ordersarrtotal->addFieldToFilter('status', $filter);
                } elseif ($filtertype == 'fulfillment') {
                    if ($filter =='fulfilled') {
                        $ordersarr->addFieldToFilter('ma_tracking', ['neq' => '']);
                        $ordersarrtotal->addFieldToFilter('ma_tracking', ['neq' => '']);
                    } elseif ($filter =='unfulfilled') {
                        $ordersarr->addFieldToFilter('ma_tracking', ['null' => true], 'left');
                        $ordersarrtotal->addFieldToFilter('ma_tracking', ['null' => true], 'left');
                    }
                } elseif ($filtertype == 'date') {
                    if ($filter == 'today') {
                        $ordersarr->addFieldToFilter(
                            'created_at',
                            [
                                'from'     => strtotime($this->date->gmtDate('Y-m-d 00:00:00')),
                                'to'       => strtotime($this->date->gmtDate('Y-m-d 23:59:59')),
                                'datetime' => true
                            ]
                        );
                        $ordersarrtotal->addFieldToFilter(
                            'created_at',
                            [
                                'from'     => strtotime($this->date->gmtDate('Y-m-d 00:00:00')),
                                'to'       => strtotime($this->date->gmtDate('Y-m-d 23:59:59')),
                    ...
                    } elseif ($filter == 'less_than') {
                        if ($params['date'] != '') {
                            $date = $this->date->gmtDate('Y-m-d', strtotime($params['date']));
                            $ordersarr->addFieldToFilter('created_at', ['lt' => $date]);
                            $ordersarrtotal->addFieldToFilter('created_at', ['lt' => $date]);
                        }
                    } elseif ($filter == 'greater_than') {
                        if ($params['date'] != '') {
                            $date = $this->date->gmtDate('Y-m-d', strtotime($params['date']));
                            $ordersarr->addFieldToFilter('created_at', ['gt' => $date]);
                            $ordersarrtotal->addFieldToFilter('created_at', ['gt' => $date]);
                        }
                    }
                }
                ...
                foreach ($ordersarr as $key => $order) {
                    ...
                    if ($countrycode != $orderAddress['country_id']) {
                        $orders[20-$key]['international'] = true;
                    }
                }
                ...
                $error = false;
            } elseif (isset($params['getestcost'])) {
                $msg = $this->getestcost($params);
                $error = false;
            } elseif (isset($params['pLable'])) {
                $msg = $this->printlabel($params);
                $error = false;
            } elseif (isset($params['dLable'])) {
                $msg = $this->downloadLbl($params);
                $error = false;
            } elseif (isset($params['retLable'])) {
                $msg = $this->printreturnlbl($params);
                $error = false;
            }
        }
    ...
    public function getestcost($postParams)
    {
        $uname = $this->scopeConfig->getValue('malca/amit/username');
        $pwd = $this->scopeConfig->getValue('malca/amit/password');
        $sCode = $this->scopeConfig->getValue('malca/amit/stationcode');
        $error='';

        $arrLogin = [
            'UserName' => trim($uname),
            'UserPassword'=> trim($pwd),
            'UserStationCode'=>trim($sCode),
            'SourceTypeId'=>6
        ];
        ...
        $ordersarr = $this->orderCollectionFactory->create()->addAttributeToSelect('*')
            ->addFieldToFilter('entity_id', $postParams['o_id']);
        foreach ($ordersarr as $key => $order) {
            if ($order->getShippingAddress()) {
                $oAdd= $order->getShippingAddress()->getData();
                $oAdd['RegionCode'] = $order->getShippingAddress()->getRegionCode();
            } else {
                $oAdd= $order->getBillingAddress()->getData();
                $oAdd['RegionCode'] = $order->getBillingAddress()->getRegionCode();
            }

            $shippingCharge = $order->getData('shipping_incl_tax');
            $subTotal = $order->getGrandTotal() - (float)$shippingCharge;
            $cCode = $order->getOrderCurrencyCode();
            $orid= $order->getIncrementId();
            $sAdd = $order->getShippingMethod();
        }
        try {
            $countrycode = $this->scopeConfig->getValue('shipping/origin/country_id');

            if ($countrycode != $oAdd['country_id']) {
                $method = 'GetMAEXEstimatedPriceForInternationalShipment';
                $actionHeader = new \SoapHeader('http://tempuri.org/', $method, $arrLogin);
                $client->__setSoapHeaders($actionHeader);

                $params = $this->internationalShipParams($arrLogin, $oAdd, $subTotal, $cCode, $orid, $sAdd);
                
                $result = $client->__soapCall('GetMAEXEstimatedPriceForInternationalShipment', [$params]);
                $result= json_encode($result);
                $result= json_decode($result, true);
                $chargesDetails  = $result["GetMAEXEstimatedPriceForInternationalShipmentResult"];
            } else {
                $actionHeader=new \SoapHeader('http://tempuri.org/', 'GetMAEXEstimatedPriceForDomesticShipment', true);
                $client->__setSoapHeaders($actionHeader);
                $params = $this->domesticShipParams($arrLogin, $oAdd, $subTotal, $cCode, $sAdd, $orid);

                $result = $client->__soapCall('GetMAEXEstimatedPriceForDomesticShipment', [$params]);
                $result= json_encode($result);
                $result= json_decode($result, true);
    ...
    public function printlabel($postParams)
    {

        $resp =[];
        foreach ($postParams['o_id'] as $key => $oId) {
            $res = $this->multiLblPrint($oId);
            $resp[$key]  = $res;
        }

        return json_encode($resp);
    }
    public function multiLblPrint($oId)
    {
        $error = $file = $tracknumber ='';
        $flag = true;
        $order = $this->_order->load($oId);
        
        $orid= $order->getIncrementId();
        
        $tracknumber = $order->getMa_tracking();
        $courierType = $order->getMa_couriertype();
        $uname = $this->scopeConfig->getValue('malca/amit/username');
        $pwd = $this->scopeConfig->getValue('malca/amit/password');
        $sCode = $this->scopeConfig->getValue('malca/amit/stationcode');
        ...
        if ($tracknumber == '') {
            ...
            if ($countrycode != $oAdd['country_id']) {
                ...
                $actionHeader = new \SoapHeader('http://tempuri.org/', 'CreateMAEXInternationalShipment', true);
                $client->__setSoapHeaders($actionHeader);
                
                $result = $client->__soapCall('CreateMAEXInternationalShipment', [$params]);
                $result= json_decode(json_encode($result), true);
                $trackShipment  = $result['CreateMAEXInternationalShipmentResult'];
            } else {
                $params = $this->domesticShipParams($arrLogin, $oAdd, $subTotal, $cCode, $sAdd, $orid);
                $actionHeader = new \SoapHeader('http://tempuri.org/', 'CreateMAEXDomesticShipment', true);
                $client->__setSoapHeaders($actionHeader);
                
                $result = $client->__soapCall('CreateMAEXDomesticShipment', [$params]);
                $result= json_decode(json_encode($result), true);
            ...
            if ($trackShipment['NotificationType'] == 'Success') {
                ...
                $data = ['ma_tracking' => $tracknumber, 'ma_couriertype' => $courierType];
                try {
                    $orderModel = $this->_order->load($oId)
                        ->setMa_tracking($tracknumber)
                        ->setMa_couriertype($courierType);
                ...
                $this->addLog($client, $error, $orid);
                $flag = false;
            }
        }
        if ($tracknumber != '') {
            try {
                $actionHeader = new \SoapHeader('http://tempuri.org/', 'GetMAEXDocument', true);
                $client->__setSoapHeaders($actionHeader);
                $arrLogin['SourceTypeId'] = 6;

                $labelsparam = ['UserAuthentication' => $arrLogin,
                        'TrackingNumber' => $tracknumber,
                        'DocumentType' => 'Label'
                    ];

                $result = $client->__soapCall('GetMAEXDocument', [$labelsparam]);
                $result = $result->GetMAEXDocumentResult;
                if ($result->NotificationType == 'Success') {
                    $filepath = '';
                    $filename = $orid."_Label.pdf";
                    $file = $filename;
        ...
        $resp['ma_couriertype'] = $courierType;
        
        return $resp;
    }
    public function downloadLbl($postParams)
    {
        if (isset($postParams['single'])) {
            $filename = explode("/", $postParams['files']);
            ...
            if ($filename) {
                try {
                    $this->getResponse()
                        ...
                        ->setHeader('Content-Length', filesize($postParams['files']))
                        ->setHeader('Content-Disposition', 'attachment' . '; filename=' . basename($postParams['files']));
                    $this->getResponse()->clearBody();
                    $this->getResponse()->sendHeaders();
                    readfile($postParams['files']);
                    unlink($postParams['files']);
                } catch (\Exception $e) {
                    $this->_getSession()->addError($e->getMessage());
                }
            }
        } else {
            $fileNames = $postParams['files'];
            $archivefile='Malca-labels.zip';
            $zip = new \ZipArchive();
            if ($zip->open($archivefile, \ZIPARCHIVE::CREATE)!==true) {
                ...
                $this->getResponse()
                    ...
                    ->setHeader('Content-Length', filesize($archivefile))
                    ->setHeader('Content-Disposition', 'attachment' . '; filename=' . basename($archivefile));
                $this->getResponse()->clearBody();
                $this->getResponse()->sendHeaders();
    ...
    public function printreturnlbl($postParams)
    {
        $error = $file = $tracknumber ='';
        $flag = true;
        if ($postParams['o_id'] != '') {
            $order = $this->_order->load($postParams['o_id']);
            $orid= $order->getIncrementId();
            $uname = $this->scopeConfig->getValue('malca/amit/username');
            $pwd = $this->scopeConfig->getValue('malca/amit/password');
            $sCode = $this->scopeConfig->getValue('malca/amit/stationcode');
            ...
            if ($countrycode == $orderAddress['country_id']) {
                try {
                    ...
                    $actionHeader = new \SoapHeader('http://tempuri.org/', 'CreateMAEXReturnDomesticShipment', true);
                    $client->__setSoapHeaders($actionHeader);
                    ...
                    $result = $client->__soapCall('CreateMAEXReturnDomesticShipment', [$labelsparam]);
                    $result = $result->CreateMAEXReturnDomesticShipmentResult;
                    if ($result->NotificationType == 'Success') {
                        $filename = $orid."_Return_Label.pdf";
                        $file = $filename;
                        //@codingStandardsIgnoreStart
                        $data = base64_encode($result->FileStream);
                        $decoded = base64_decode($data);
    ...
    public function domesticShipParams($userarrys, $sAdd, $subTotal, $currencyCode, $shippingmethod, $orid)
    {
        ...
        $params = [
            ...
            'ShipmentDetails' => [
                ...
                'IsEcommerce' => true,
                'ConsigneeCompanyName' =>($sAdd['company'] !='') ? $sAdd['company'] : $sAdd['firstname'],
                'ConsigneeName' => $sAdd['firstname']." ".$sAdd['lastname'],
                'ConsigneeEmail' => $sAdd['email'],
                'ConsigneeNameOnLabel' => ($sAdd['company'] !='') ? $sAdd['company'] : $sAdd['firstname']." ".$sAdd['lastname'],
                'ConsigneePhoneCountryCode' => $this->getCountryPhoneCode($sAdd['country_id']),
                'ConsigneePhoneNumber' => $sAdd['telephone'],
                'ConsigneeZipCode' => $sAdd['postcode'],
                'PackageWeight' => 1,
                'PickUpDateTime' => $this->date->gmtDate('Y-m-d'),
                'PickupMethod' => 'PickupByCourier',
                'LiabilityValueUSD' => $this->convertPrice($subTotal, $currencyCode),
                'PackageLength' => '',
                'PackageWidth' => '',
                'PackageHeight' => '',
                'SelectedShippingMethodDesc' => $shippingmethod,
                'ConsigneeReference' => $orid,
                'InvoiceNumber' => $orid,
                'EcommercePlatformID' => 'Magento2',
                'EcommerceStoreUrl' => $baseUrl
            ]
        ];
        if (strlen($sAdd['street'])>35) {
            $adrArr = str_split($sAdd['street'], 35);
            foreach ($adrArr as $key => $value) {
                $i=$key+1;
                $params['ShipmentDetails']['ConsigneeAddress'.$i] = $value;
            }
        } else {
            $params['ShipmentDetails']['ConsigneeAddress1'] = $sAdd['street'];
        }

        return $params;
    }
    public function internationalShipParams($userarrys, $sAdd, $subTotal, $currencyCode, $orid, $shippingmethod)
    {
        $objectManager =  \Magento\Framework\App\ObjectManager::getInstance();
        $storeManager = $objectManager->get('\Magento\Store\Model\StoreManagerInterface');
        $store = $storeManager->getStore();
        ...
        $baseUrl = $store->getBaseUrl();
        $params = ['UserAuthentication' => $userarrys,
                'ShipmentDetails' => [
                    'ConsigneeCountryCode' =>$sAdd['country_id'],
                    'CourierType' => 'None',
                    'ServiceType' => 'None',
                    ...
                    'IsResidentialAddress' => true,
                    'ConsigneeCompanyName' =>($sAdd['company'] != '' ? $sAdd['company'] : $sAdd['firstname']),
                    'ConsigneeName' => $sAdd['firstname']." ".$sAdd['firstname'],
                    'ConsigneeNameOnLabel' => ($sAdd['company'] !='') ? $sAdd['company'] : $sAdd['firstname']." ".$sAdd['lastname'],
                    'ConsigneePhoneCountryCode' => $this->getCountryPhoneCode($sAdd['country_id']),
                    'ConsigneeCity' => $sAdd['city'],
                    'ConsigneePhoneNumber' => $sAdd['telephone'],
                    'ConsigneeZipCode' =>$sAdd['postcode'],
                    'ConsigneeStateCode' =>$sAdd['RegionCode'],
                    'ConsigneeReference' => $orid,
                    'PackageWeight' => 1,
                    'PickUpDateTime' => $this->date->gmtDate('Y-m-d'),
                    'PickupMethod' => 'PickupByCourier',
                    'PickupState' =>$sAdd['RegionCode'],
                    'PickupPhoneNumber' => $sAdd['telephone'],
                    'LiabilityValueUSD' => $this->convertPrice($subTotal, $currencyCode),
                    'PackageLength' => '',
                    'PackageWidth' => '',
                    'PackageHeight' => '',
    ...
    public function convertPrice($amt, $fromCur)
    {
        //@codingStandardsIgnoreStart
        ...
        return ($amt * $arr[$fromCur]) / $arr['USD'];
    }
    protected function addLog($client, $message, $orderNumber = '')
    {
        $params = [
            "LogDetails"=>[
              ...
              "LogCreatedDateTime"=>$this->date->gmtDate('Y-m-d')
            ],
            "ApplicationId"=>969
        ];

        try {
            $actionHeader = new \SoapHeader('http://tempuri.org/', 'AddMAEXEcommerceAppLog', true);
            $client->__setSoapHeaders($actionHeader);

            $result = $client->__soapCall('AddMAEXEcommerceAppLog', [$params]);
            $result= json_decode(json_encode($result), true);
        } catch (\Exception $e) {
            $e->getMessage();
        }
